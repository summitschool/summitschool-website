...
import { buildHubFormCompleteUrl } from '../_shared/hub-form-email.ts';

const SUPABASE_URL = Deno.env.get('SUPABASE_URL')!;
const SUPABASE_SERVICE_ROLE_KEY = Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!;

const WEBHOOK_SECRET = Deno.env.get('DOCUSEAL_HUB_ARCHIVE_WEBHOOK_SECRET')
...
const DEFAULT_HUB_ARCHIVE_SLUGS = '3oBpb3Knk9GsNB';
const HUB_ARCHIVE_SCHOOL_YEAR = Deno.env.get('DOCUSEAL_HUB_ARCHIVE_SCHOOL_YEAR') || '2026-2027';
const HUB_ARCHIVE_CATEGORY = Deno.env.get('DOCUSEAL_HUB_ARCHIVE_CATEGORY') || 'Signed Form';
...
function parseSlugList(raw: string | undefined) {
  return (raw || DEFAULT_HUB_ARCHIVE_SLUGS)
    .split(',')
  ...
  if (slugs.length > 0 && DOCUSEAL_API_KEY) {
    try {
      ...
      if (response.ok) {
        ...
        console.error('docuseal hub archive template lookup failed', response.status, await response.text());
      }
    } catch (error) {
      console.error('docuseal hub archive template lookup error:', error);
    }
  }
...
async function configureSubmitterRedirect(submitterId: number, templateSlug?: string) {
  ...
  const slug = templateSlug || resolveTemplateSlug();
  ...
  try {
    const response = await fetch(`${DOCUSEAL_API_URL}/api/submitters/${submitterId}`, {
      ...
      body: JSON.stringify({
        completed_redirect_url: buildHubFormCompleteUrl(slug),
      }),
    });

    if (!response.ok) {
      console.error('docuseal hub archive redirect update failed', response.status, await response.text());
      return false;
    }

    return true;
  } catch (error) {
    console.error('docuseal hub archive redirect update error:', error);
    return false;
  }
}
...
async function handleFormCompleted(options: {
  ...
  const result = await archiveDocuSealSubmissionToHub({
    ...
    schoolYear: HUB_ARCHIVE_SCHOOL_YEAR,
    category: HUB_ARCHIVE_CATEGORY,
  });
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('Hub archive handled', { submissionId, templateId, familyEmail, result });
    ...
    console.error('docuseal-hub-archive-webhook error:', error);
    return new Response(JSON.stringify({
      ok: false,
